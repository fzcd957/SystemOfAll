...
public class Expression {
	...
	String updateById(ClassTableDefinition ctd) {
		...
		.append("Illness ").append("=? ").append("WHERE ").append(Id.getColumn()).append("=?");
		
		return res.toString();
	}
	
	String update1ById(ClassTableDefinition ctd) {
		StringBuffer res = new StringBuffer("UPDATE ");
		PropertyColumn Id = ctd.getId();
		res.append(ctd.getTable()).append(' ').append("SET ")
		.append("Result ").append("=? ").append("WHERE ").append(Id.getColumn()).append("=?");
		
		return res.toString();
	}
	
	String update3ById(ClassTableDefinition ctd) {
		StringBuffer res = new StringBuffer("UPDATE ");
		PropertyColumn Id = ctd.getId();
		res.append(ctd.getTable()).append(' ').append("SET ")
		.append("status ").append("=? ").append("WHERE ").append(Id.getColumn()).append("=?");
		
		return res.toString();
	}
	
	String update2ById(ClassTableDefinition ctd) {
		StringBuffer res = new StringBuffer("UPDATE ");
		PropertyColumn Id = ctd.getId();
		res.append(ctd.getTable()).append(' ').append("SET ")
		.append("money ").append("=? ").append("WHERE ").append(Id.getColumn()).append("=?");
